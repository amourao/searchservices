...
void Initialize_Data(float** indexData,float** queryData, int indexPoints, int queryPoints, int dims)
{
...
void Compute_GroundTruth(int k)
{
	gt = new int * [ nQ ];
	for(int i=0;i<nQ;i++)
	{
...
void Process()
{
	...
	for(int qIndex=0;qIndex<nQ;qIndex++)
	{
		...
		for(int i=0;i<nP;i++)
		{
			resLSH[i].index    = i;		resLSH[i].dist    = lsh.Compute_HD(  bCodeQuery_LSH[qIndex] , bCodeData_LSH[i] );
			resSH_HD[i].index  = i;		resSH_HD[i].dist  = lsh.Compute_HD(  bCodeQuery_SH[qIndex]  , bCodeData_SH[i]  );
			resSH_SHD[i].index = i;		resSH_SHD[i].dist = lsh.Compute_SHD( bCodeQuery_SH[qIndex]  , bCodeData_SH[i]  );
		}
...
int main(float** indexData,float** queryData, int indexPoints, int queryPoints, int dims)
{
	srand( (unsigned int)( time(NULL) ) );

	Stopwatch T0("");
	T0.Reset();		T0.Start();
	Initialize_Data(indexData,queryData,indexPoints,queryPoints,dims);
	T0.Stop();
