...
void awesomeIndexTesterAllV2(int argc, char *argv[]){
    bool debug = false;
    IBinImporter* importer;
	...
	arma::fmat featuresTrain;
	arma::fmat featuresValidationI;
	arma::fmat featuresValidationQ;
	arma::fmat featuresTestI;
	arma::fmat featuresTestQ;
    ...
    //bool gtLoaded = loadGT(outfileNameA,linearResults,simpleFileName,testOffset,biggestNTesI,nTesQ,biggestK);
    //if (!gtLoaded){
    //	cout << "error loading GT: " << outfileNameA << endl;
    //} else {
    //    if (debug) cout << "Loading GT ok" << endl;
    ...
    for(uint i = 0; i < indexers.size(); i++){
        ...
        if(indexToTest == 0){
            ...
            cout << nTrain << ";" << nValI << ";" << nValQ << ";" << nTesI << ";" << nTesQ << ";" << kList.size() << ";";
            for (uint ii = 0; ii < kList.size(); ii++){cout << kList.at(ii)<<";";}
            cout << featuresTestI.n_rows << ";" << atoi(parameters["startOffset"].c_str()) << endl;
            cout << "Index name;Name;Base name;Train time (ms);Index time (ms);Retrieval index;";
            for (uint ii = 0; ii < kList.size(); ii++){cout << "Query time (ms)@" << kList.at(ii) << ";mAcc@" << kList.at(ii) << ";MAP@" << kList.at(ii) << ";Dist@" << kList.at(ii) << ";";}
            cout << endl;
        }
    	...
    	if (debug) cout << "Training" << endl;
    	if(!indexers.at(i)->load(ss2.str())){
            get_timestamp(&start);
            indexers.at(i)->train(featuresTrain,featuresValidationQ,featuresValidationI);
            get_timestamp(&end);
            indexers.at(i)->save(ss2.str());
		} else {
            if (debug) cout << "loaded trained params" << endl;
 		}
		if (debug) cout << "Training ok" << endl;
        ...
        while (indexers.at(i)->hasNextRetrievalParameters()){
            ...
            retInd++;
            cout << indexers.at(i)->getIndexingParameters() << indexers.at(i)->getRetrievalParameters() << endl << endl;
